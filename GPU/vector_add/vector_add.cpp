...
int *input_b=(int *) malloc(sizeof(int));
...
int status;
 ...
 // Input buffers.
	...
	for (int k = 0; k<ceil(log2(N)); k++){
		...
		printf("loop %d, ind %d, tmp %d\n", k, input_b[0], tmp);

		status = clEnqueueWriteBuffer(queue, input_b_buf, CL_FALSE,
        0, sizeof(int), input_b, 0, NULL, &write_event[1]);
	...
	// Verify results.
   	if(fabsf(output[0] - ref_output[0]) > 1.0e-5f) {
  		printf("Failed verification @\nOutput: %f\nReference: %f\n",
            output[0], ref_output[0]);
  	}
