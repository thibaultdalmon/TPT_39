...
float input_a[N*N];
float input_b[N*N];
float output_tmp[N*N*N];
float output[N*N];
float ref_output[N*N];
cl_mem input_a_buf; // num_devices elements
cl_mem input_b_buf; // num_devices elements
cl_mem output_buf_tmp; // num_devices elements
int status;
	...
	for (unsigned k = 0; k<N; k++){
		for (unsigned l = 0; l<N; l++){
			...
			for (unsigned i = 0; i<N; i++){
				tmp += input_a[k*N+i]*input_b[i*N+l];
			}
			ref_output[k*N+l] = tmp;
		}
	}
	time (&end);
 ...
 // Input buffers.
    ...
    // Output buffer.
    output_buf_tmp = clCreateBuffer(context, CL_MEM_WRITE_ONLY,
        N*N*N* sizeof(float), NULL, &status);
	...
	size_t global_work_size[3] = {N,N,N};
    ...
    status = clSetKernelArg(kernel, argi++, sizeof(cl_mem), &output_buf_tmp);
	...
	// Read the result. This the final operation.
   	status = clEnqueueReadBuffer(queue, output_buf_tmp, CL_TRUE,
   		0, N*N*N* sizeof(float), output_tmp, 1, &kernel_event, &finish_event);
    ...
    clReleaseMemObject(output_buf_tmp);
		...
		for (unsigned i = 0; i<N && pass; ++i) {
      		if(fabsf(output[j*N+i] - ref_output[j*N+i]) > 1.0e-5f) {
        		printf("Failed verification @ index %d\nOutput: %f\nReference: %f\n",
            		j, output[j*N+i], ref_output[j*N+i]);
        		pass = false;
      		}
		}
	}
